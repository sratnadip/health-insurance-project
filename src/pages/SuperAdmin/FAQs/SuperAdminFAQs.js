...
export default function SuperAdminFAQs() {
  ...
  const handleSave = async () => {
    try {
      if (editId) {
        await axios.put(`http://localhost:8089/faq/update/${editId}`, faqData);
        alert("FAQ updated successfully!");
      } else {
        await axios.post("http://localhost:8089/faq/add", faqData);
  ...
  const handleDelete = async (id) => {
    if (window.confirm("Are you sure you want to delete this FAQ?")) {
      try {
        await axios.delete(`http://localhost:8089/faq/delete/${id}`);
